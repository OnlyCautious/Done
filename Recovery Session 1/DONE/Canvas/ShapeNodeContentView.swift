...
struct ShapeNodeContentView: View {
    ...
    // Same pattern as TextNoteContentView.autosave(_:): every keystroke saves
    // immediately so text is never lost, but only the first mutation of the
    // session registers with the undo manager, so undoing the whole edit is
    // still one step rather than one per character.
